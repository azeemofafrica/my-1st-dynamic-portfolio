...
const skillInfo = {
    ...
    "REACT": "React is a JavaScript library for building interactive user interfaces—especially single-page applications (SPAs).",
    "FIGMA": "Figma is a cloud-based design tool used to create UI/UX mockups, wireframes, and prototypes."
};
skillButtons.forEach(button => {
    button.addEventListener('click', () => {
        const skill = button.dataset.skill; 
        skillDescription.textContent = skillInfo[skill];
        skillDescription.style.color = 'BLACK'; 
    });
});
   ...
   const darkmode = new Darkmode({
    ...
    document.getElementById('darkmode-toggle').addEventListener('click', function () {
      document.querySelectorAll('.dark-toggle-target').forEach(section => {
        section.classList.toggle('darkmode-active');
      });
    });
